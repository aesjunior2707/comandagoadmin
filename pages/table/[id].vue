  ...
  <div class="min-h-screen bg-gray-50">
        ...
        <div class="flex items-center space-x-4">
          ...
          <div>
            <h1 class="text-xl font-semibold text-gray-900">
              Mesa {{ tableInfo?.description }}
            </h1>
            <p class="text-sm text-gray-600">
              {{ orders.length }}
              {{ orders.length === 1 ? "item" : "itens" }} no pedido
            </p>
          </div>
        </div>
            ...
            <div class="p-6">
              ...
              <div v-if="orders.length === 0" class="text-center py-12">
                ...
                <p class="text-gray-600 mb-6">
                  Esta mesa não possui itens no momento
                </p>
                <NuxtLink
                  :to="`/pos?table=${tableInfo?.id}`"
                  class="btn-primary"
                >
                  <PlusIcon class="w-4 h-4 mr-2" />
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-2 text-sm">
              ...
              <div v-if="orders.length > 0" class="flex justify-between">
                <span class="text-gray-600">Primeiro Pedido</span>
                <span class="font-medium">{{
                  formatTime(firstOrderTime)
                }}</span>
              </div>
            </div>
          </div>
      ...
      <div class="bg-white rounded-xl max-w-md w-full p-6">
        <div class="text-center mb-6">
          ...
          <p class="text-gray-600">
            Tem certeza que deseja remover todos os itens da Mesa
    ...
    >
      <div class="bg-white rounded-xl max-w-md w-full p-8 text-center">
        ...
        <p class="text-gray-600 mb-6">
          A conta da Mesa {{ tableInfo?.description }} foi finalizada com
          sucesso
        </p>
        <div class="space-y-3">
...
const subtotal = computed(() => {
  return orders.value.reduce((sum, item) => sum + Number(item.total_price), 0);
});
...
const removeAllItems = () => {
  RestaurantStore.clear_all_table(tableInfo.value.id)
    .then(() => {
