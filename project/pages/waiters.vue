...
const onDutyWaiters = computed(
  () => waiters.value.filter((w) => w.status === "on-duty").length
);
const onBreakWaiters = computed(
  () => waiters.value.filter((w) => w.status === "on-break").length
);
const offDutyWaiters = computed(
  () => waiters.value.filter((w) => w.status === "off-duty").length
);
