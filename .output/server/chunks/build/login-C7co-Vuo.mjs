...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div><h2 class="mt-6 text-3xl font-bold text-gray-900">Bem vindo de Volta</h2><p class="mt-2 text-sm text-gray-600">Fa\xE7a login na sua conta de Administrador ComandaGo</p></div><div class="card"><div class="card-body"><form class="space-y-6"><div><label for="email" class="block text-sm font-medium text-gray-700 mb-2"> Endere\xE7o de e-mail / Usu\xE1rio </label><input id="email"${ssrRenderAttr("value", unref(form).email)} type="email" autocomplete="email" required class="${ssrRenderClass([{ "input-error": unref(errors).email }, "input-field"])}" placeholder="admin@comandago.com">`);
      ...
      _push(`<div class="ml-3"><h3 class="text-sm font-medium text-blue-800">Demo Credentials</h3><div class="mt-2 text-sm text-blue-700"><p><strong>Email:</strong> admin@comandago.com</p><p><strong>Password:</strong> admin123</p></div></div></div></div><div class="text-center"><p class="text-xs text-gray-500"> \xA9 2025 ComandaGo Admin. Professional Restaurant Management System. </p></div></div></div>`);
    };
  }
};
...
//# sourceMappingURL=login-C7co-Vuo.mjs.map
